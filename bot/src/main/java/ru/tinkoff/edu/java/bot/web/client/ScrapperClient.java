...
public class ScrapperClient {
    ...
    public ResponseEntity getLinks(long chatId) {
        ...
        {
            try {
                return new ResponseEntity(mapper.readValue(response.getBody().toString(), ListLinksResponse.class), HttpStatus.OK);
            }
            catch (Exception e)
            {
    ...
    public ResponseEntity<?> addLink(long chatId, AddLinkRequest request)
    {
        ...
        try {
            ...
            {
                return new ResponseEntity<>(mapper.readValue(tempres.getBody().toString(),ApiErrorResponse.class),HttpStatus.BAD_REQUEST);
            }
            else
            {
                return new ResponseEntity(mapper.readValue(tempres.getBody().toString(), LinkResponse.class),HttpStatus.OK);
            }
    ...
    public ResponseEntity<?> deleteLink(long chatId, RemoveLinkRequest request)
    {
        ...
        try
        {
            ...
            {
                return new ResponseEntity<>(mapper.readValue(result.getBody().toString(),ApiErrorResponse.class),HttpStatus.BAD_REQUEST);
            }
            else
            {
                return new ResponseEntity<>(mapper.readValue(result.getBody().toString(),LinkResponse.class),HttpStatus.OK);
            }
        }
        catch(Exception e)
        {
    ...
    public ResponseEntity<?> registerChat(long ChatId)
    {
        ...
        ResponseEntity response = client
                .post()
                .uri(baseurl + "/tg-chat/" + Long.toString(ChatId))
                .retrieve()
                .onStatus(status -> status.value() == 400,resp -> Mono.empty())
