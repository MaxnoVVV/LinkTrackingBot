...
public class UntrackCommand extends Command {
  ...
  public CommandResponse proccess(Update update) {
    if (update.message().text().equals("/untrack")) {
      log.info(String.format("Update %d with %s message is /untrack command", update.updateId(),
          update.message().text()));

      return new SuccessCommandResponse("Отправьте ссылку для отмены отслеживания", true);
    } else {
      log.info(String.format("Update %d with %s message is not /untrack command", update.updateId(),
          update.message().text()));
  ...
  public static SendMessage stopTrack(Update update) {
    ResponseEntity<?> response = client.deleteLink(update.message().chat().id(),
    ...
    if (response.getStatusCode().is2xxSuccessful()) {
      ...
      return new SendMessage(update.message().chat().id(),
          "Ссылка некорректна или не отслеживается");
    } else {
      return new SendMessage(update.message().chat().id(),
